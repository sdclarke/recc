...
TEST(FileUtilsTest, CreateDirectoryRecursive)
{
    ...
    FileUtils::create_directory_recursive(name.c_str());
...
TEST(FileUtilsTest, Executable)
{
    ...
    ASSERT_THROW(FileUtils::is_executable(fileName.c_str()), std::exception);
    ASSERT_THROW(FileUtils::make_executable(fileName.c_str()), std::exception);

    FileUtils::write_file(fileName.c_str(), ""); // Create a test file.

    EXPECT_FALSE(FileUtils::is_executable(fileName.c_str()));
    FileUtils::make_executable(fileName.c_str());
    EXPECT_TRUE(FileUtils::is_executable(fileName.c_str()));
}
...
TEST(FileUtilsTest, FileContents)
{
    ...
    EXPECT_THROW(FileUtils::get_file_contents(fileName.c_str()),
                 std::exception);

    FileUtils::write_file(fileName.c_str(), "File contents");
    EXPECT_EQ(FileUtils::get_file_contents(fileName.c_str()), "File contents");

    FileUtils::write_file(fileName.c_str(), "Overwrite, don't append");
    EXPECT_EQ(FileUtils::get_file_contents(fileName.c_str()),
...
{
    EXPECT_TRUE(FileUtils::has_path_prefix("/a/b/c/", "/a/b"));
    EXPECT_TRUE(FileUtils::has_path_prefix("/a/b/c/", "/a/b/"));
    EXPECT_TRUE(FileUtils::has_path_prefix("/a/b/c", "/a/b"));
    EXPECT_TRUE(FileUtils::has_path_prefix("/a/b/c", "/a/b/"));

    EXPECT_FALSE(FileUtils::has_path_prefix("/a/c/d", "/a/b/"));

    EXPECT_FALSE(FileUtils::has_path_prefix("/a/boo", "/a/b/"));
    EXPECT_FALSE(FileUtils::has_path_prefix("/a/boo", "/a/b"));
    EXPECT_FALSE(FileUtils::has_path_prefix("/a/boo", "/a/b/a/boo"));
    EXPECT_FALSE(FileUtils::has_path_prefix("/a/boo", "/a/b/a/boo/"));

    EXPECT_TRUE(FileUtils::has_path_prefix("/a/../b/", "/a"));
    EXPECT_TRUE(FileUtils::has_path_prefix("/a/../b/", "/a/"));
    EXPECT_TRUE(FileUtils::has_path_prefix("/a/../b", "/a"));
    EXPECT_TRUE(FileUtils::has_path_prefix("/a/../b", "/a/"));
}
...
TEST(HasPathPrefixTest, RelativePaths)
{
    ...
    EXPECT_TRUE(FileUtils::has_path_prefix("a/b/c", "a/b"));
    EXPECT_TRUE(FileUtils::has_path_prefix("a/b/c", "a/b/"));
    EXPECT_TRUE(FileUtils::has_path_prefix("/a/b/c", "/a/b/c"));
...
TEST(HasPathPrefixesTest, SystemPaths)
{
...
{
    EXPECT_EQ("", FileUtils::make_path_relative(std::string(""),
                                                "/some/working/directory"));
    EXPECT_EQ("", FileUtils::make_path_relative(std::string(""),
                                                "/some/working/directory"));
    EXPECT_EQ("test", FileUtils::make_path_relative(
                          std::string("test"), "/some/working/directory"));
    EXPECT_EQ("test/long/path",
              FileUtils::make_path_relative(std::string("test/long/path"),
                                            "/some/working/directory"));
    EXPECT_EQ("some/path",
              FileUtils::make_path_relative(std::string("some/path"),
                                            "/some/working/directory"));
}
...
TEST_F(MakePathRelativeTest, DoNothingIfWorkingDirectoryNull)
{
    EXPECT_EQ("/test/directory/",
              FileUtils::make_path_relative(std::string("/test/directory/"),
                                            nullptr));
    EXPECT_EQ("/test/directory/", FileUtils::make_path_relative(
                                      std::string("/test/directory/"), ""));
    EXPECT_EQ("/test",
              FileUtils::make_path_relative(std::string("/test"), ""));
}

TEST_F(MakePathRelativeTest, WorkingDirectoryIsPathPrefix)
{
    EXPECT_EQ("path", FileUtils::make_path_relative(
...
{
    EXPECT_EQ(".", FileUtils::make_path_relative(
...
{
    EXPECT_EQ("../tests", FileUtils::make_path_relative(
...
{
    EXPECT_EQ("..", FileUtils::make_path_relative(std::string("/a/b/c"),
                                                  "/a/b/c/d"));
    EXPECT_EQ("..", FileUtils::make_path_relative(std::string("/a/b/c"),
                                                  "/a/b/c/d/"));
    EXPECT_EQ("../", FileUtils::make_path_relative(std::string("/a/b/c/"),
    ...
    EXPECT_EQ("../../..",
              FileUtils::make_path_relative(std::string("/a"), "/a/b/c/d"));
    EXPECT_EQ("../../..",
              FileUtils::make_path_relative(std::string("/a"), "/a/b/c/d/"));
    EXPECT_EQ("../../../",
              FileUtils::make_path_relative(std::string("/a/"), "/a/b/c/d"));
...
{
    EXPECT_EQ("../e", FileUtils::make_path_relative(std::string("/a/b/c/e"),
                                                    "/a/b/c/d"));
    EXPECT_EQ("../e", FileUtils::make_path_relative(std::string("/a/b/c/e"),
                                                    "/a/b/c/d/"));
    EXPECT_EQ("../e/", FileUtils::make_path_relative(std::string("/a/b/c/e/"),
    ...
    EXPECT_EQ("../e/f/g", FileUtils::make_path_relative(
...
TEST_F(MakePathRelativeTest, PathOutsideProjectRoot)
{
    ...
    EXPECT_EQ(
        ...
        FileUtils::make_path_relative(
            std::string("/home/nobody/include/foo.h"), "/home/nobody/test"));
    EXPECT_EQ(
        "/home/nobody/include/foo.h",
        FileUtils::make_path_relative(
            std::string("/home/nobody/include/foo.h"), "/home/nobody/test"));
...
{
    EXPECT_EQ("", FileUtils::make_path_absolute("", "/a/b/c/"));
    EXPECT_EQ("/a/b/c/", FileUtils::make_path_absolute("/a/b/c/", "/d/"));
}
...
{
    EXPECT_EQ(FileUtils::parent_directory_levels(""), 0);
    EXPECT_EQ(FileUtils::parent_directory_levels("/"), 0);
    EXPECT_EQ(FileUtils::parent_directory_levels("."), 0);
    EXPECT_EQ(FileUtils::parent_directory_levels("./"), 0);

    EXPECT_EQ(FileUtils::parent_directory_levels(".."), 1);
    EXPECT_EQ(FileUtils::parent_directory_levels("../"), 1);
    EXPECT_EQ(FileUtils::parent_directory_levels("../.."), 2);
    EXPECT_EQ(FileUtils::parent_directory_levels("../../"), 2);

    EXPECT_EQ(FileUtils::parent_directory_levels("a/b/c.txt"), 0);
    EXPECT_EQ(FileUtils::parent_directory_levels("a/../../b.txt"), 1);
    EXPECT_EQ(FileUtils::parent_directory_levels(
                  "a/../../b/c/d/../../../../test.txt"),
              2);
}
...
{
    EXPECT_EQ(FileUtils::last_n_segments("abc", 0), "");
    EXPECT_EQ(FileUtils::last_n_segments("abc", 1), "abc");
    EXPECT_ANY_THROW(FileUtils::last_n_segments("abc", 2));
    EXPECT_ANY_THROW(FileUtils::last_n_segments("abc", 3));
    ...
    EXPECT_EQ(FileUtils::last_n_segments("/a/bc", 0), "");
    EXPECT_EQ(FileUtils::last_n_segments("/a/bc", 1), "bc");
    EXPECT_EQ(FileUtils::last_n_segments("/a/bc", 2), "a/bc");
    EXPECT_ANY_THROW(FileUtils::last_n_segments("/a/bc", 3));

    EXPECT_EQ(FileUtils::last_n_segments("/a/bb/c/dd/e", 0), "");
    EXPECT_EQ(FileUtils::last_n_segments("/a/bb/c/dd/e", 1), "e");
    EXPECT_EQ(FileUtils::last_n_segments("/a/bb/c/dd/e", 2), "dd/e");
    EXPECT_EQ(FileUtils::last_n_segments("/a/bb/c/dd/e", 3), "c/dd/e");
    EXPECT_EQ(FileUtils::last_n_segments("/a/bb/c/dd/e", 4), "bb/c/dd/e");
    EXPECT_EQ(FileUtils::last_n_segments("/a/bb/c/dd/e", 5), "a/bb/c/dd/e");
    EXPECT_ANY_THROW(FileUtils::last_n_segments("/a/bb/c/dd/e", 6));
...
TEST(FileUtilsTest, JoinNormalizePath)
{
    ...
    EXPECT_EQ(FileUtils::join_normalize_path(base, extension), proper);
    // neither have slashes
    EXPECT_EQ(FileUtils::join_normalize_path(base.substr(0, bLast),
                                             extension.substr(1)),
              proper);
    // only base has slash
    EXPECT_EQ(FileUtils::join_normalize_path(base, extension.substr(1)),
              proper);
    // only extension has slash
    EXPECT_EQ(FileUtils::join_normalize_path(base.substr(0, bLast), extension),
              proper);
    // extension empty, should just normalize base
    EXPECT_EQ(FileUtils::join_normalize_path(base, ""), base.substr(0, bLast));
    // base empty, should just normalize extension
    EXPECT_EQ(FileUtils::join_normalize_path("", extension), extension);
    // both empty, should return empty string
    EXPECT_EQ(FileUtils::join_normalize_path("", ""), "");
}
...
TEST(FileUtilsTest, ExpandPath)
{
    ...
    EXPECT_EQ(FileUtils::expand_path("~/"), newHome);
    EXPECT_EQ(FileUtils::expand_path("~/fake_file"), newHome + "/fake_file");
    EXPECT_EQ(FileUtils::expand_path("fake_dir/fake_file"),
              "fake_dir/fake_file");

    setenv("HOME", "", true);
    ASSERT_THROW(FileUtils::expand_path("~/"), std::runtime_error);
    EXPECT_EQ(FileUtils::expand_path("fake_dir/fake_file"),
              "fake_dir/fake_file");
    EXPECT_EQ(FileUtils::expand_path(""), "");
...
{
    EXPECT_EQ(false, FileUtils::is_absolute_path("../hello"));
    EXPECT_EQ(true, FileUtils::is_absolute_path("/../hello/"));
    EXPECT_EQ(false, FileUtils::is_absolute_path(""));
    EXPECT_EQ(true, FileUtils::is_absolute_path("/hello/world"));
}
...
TEST(PathRewriteTest, SimpleRewriting)
{
    ...
    ASSERT_EQ("/hello/file.txt",
              FileUtils::resolve_path_from_prefix_map(test_path));
    ...
    ASSERT_EQ(test_path, FileUtils::resolve_path_from_prefix_map(test_path));
}
